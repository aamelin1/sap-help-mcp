...
| Extension fails to install on Windows, mentioning `cryptography`, Rust, or a missing `link.exe` | A 32-bit Python was picked up during install. `cryptography` — a transitive dependency of every MCP server SDK, unused by this server — ships no 32-bit Windows wheel, so it is compiled from source and needs the MSVC linker. 1.0.3 asks uv for a 64-bit interpreter of its own to head this off; if you still hit it, **use option B**, which is unaffected. Installing Visual Studio Build Tools also works, but it is several gigabytes for nothing. |
| Install fails with `Missing expected target directory for Python minor version link` | A known uv bug on Windows ([astral-sh/uv#19622](https://github.com/astral-sh/uv/issues/19622)): a dangling junction in uv's managed-Python store that uv cannot repair by itself, usually left behind by an interrupted download. Delete the link and let uv recreate it — in `cmd`: `rmdir "%APPDATA%\uv\python\cpython-3.13-windows-x86_64-none"` then `uv python install 3.13`. Note `rmdir` without `/s`: that removes the junction, not what it points at. If it persists, `rmdir /s /q "%APPDATA%\uv\python"` and install again. |
