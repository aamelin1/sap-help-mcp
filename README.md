...
| Extension fails to install on Windows, mentioning `cryptography`, Rust, or a missing `link.exe` | A 32-bit Python was picked up during install. `cryptography` — a transitive dependency of every MCP server SDK, unused by this server — ships no 32-bit Windows wheel, so it is compiled from source and needs the MSVC linker. **Use option B instead**; it is unaffected. Installing Visual Studio Build Tools also works, but it is several gigabytes for nothing. Tracked in the changelog under Unreleased. |
